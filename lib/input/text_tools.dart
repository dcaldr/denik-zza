...
class TextTools {
  ...
  static String normText(String inText) {
    inText =
        inText.toLowerCase().trim(); // TODO: consider removing all whitespace
  ...
  static DateTime? parseDate(String date) {
    ...
    try {
      ...
      _logger
          .d('Standard DateTime.parse failed for: $date, trying custom parser');
      parsedDate = null;
    }
  ...
  static DateTime? myParseDate(String date) {
    ...
    if (splitted.length != 3) {
      _logger.w(
          'Date parsing failed - invalid format (expected 3 components): $date');
      return null;
    }
    ...
    try {
      ...
      if (!_isValidDate(day, month, year)) {
        _logger.w(
            'Date parsing failed - invalid date components: day=$day, month=$month, year=$year for input: $date');
        return null;
      }
  ...
  // =====================================================
  // Gender Logic (Shared between CSV Review and Registration)
  // =====================================================

  static const Set<String> _maleTokens = <String>{
    '1',
    'm',
    'muz',
    'muž',
  ...
  static const Set<String> _femaleTokens = <String>{
    '2',
    'z',
    'zena',
    'žena',
  ...
  /// Formats gender value for display (e.g. "1" -> "Muž")
  static String formatGenderLabel(String rawValue) {
    final String normalized = normText(rawValue);
  ...
  /// Checks if the value is a recognized gender token
  static bool isRecognizedGenderToken(String value) {
    final String normalized = normText(value);
    if (normalized.isEmpty) {
      return false;
    }
    return _maleTokens.contains(normalized) ||
        _femaleTokens.contains(normalized);
  }
}
