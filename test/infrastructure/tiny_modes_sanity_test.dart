  ...
  group('Tiny mode sanity checks', () {
    ...
    group('persist mode', () {
      test('persists data across reopen with per-run directory', () async {
        ...
        final dbPath = await TestOutputManager.getDatabasePath(fname, useRunDir: true);

        // Create DB, write a record into a tiny test table, close, reopen, and verify
        var db = AppDatabase(dbPath);
        await db.customStatement('CREATE TABLE IF NOT EXISTS __tiny_test (id INTEGER PRIMARY KEY, note TEXT)');
        await db.customStatement("INSERT INTO __tiny_test (note) VALUES ('hello')");
        await db.close();

        db = AppDatabase(dbPath);
        final row = await db.customSelect('SELECT COUNT(*) AS c FROM __tiny_test').getSingle();
    ...
    group('production mode safety', () {
      test('throws when attempting to initialize without confirmation', () async {
