...
void main() {
  ...
  group('Hardcoded Setup Example Tests', () {
    ...
    test('should have created test event "Test Test Test"', () async {
      ...
      final testEvent =
          events.firstWhere((event) => event.actionTitle == 'Test Test Test');
      expect(testEvent.actionTitle, equals('Test Test Test'));
      expect(testEvent.actionDescription, contains('českými účastníky'));
    });

    test('should have created 10 Czech participants with cultural references',
        () async {
    ...
    test('should have created Czech medical records with cultural easter eggs',
        () async {
      final participants = await database.select(database.participants).get();
      ...
      final recordWithEasterEgg = records.firstWhere((record) =>
          record.description.contains('má velrybí stoličku a hodně ho bolí'));
    ...
    test('should be ready for further testing', () async {
      ...
      for (final participant in participants) {
        final records = await (database.select(database.records)
              ..where((tbl) => tbl.participantFK.equals(participant.id)))
            .get();
