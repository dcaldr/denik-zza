...
void main() {
  ...
  group('Date Parsing',(){
    ...
    test('date validation robustness - invalid days', () {
      ...
      expect(TextTools.myParseDate('31.09.2020'), null); // September has 30 days
      expect(TextTools.myParseDate('31.11.2020'), null); // November has 30 days
      
      // February edge cases
      expect(TextTools.myParseDate('30.02.2020'), null); // Feb never has 30 days
      expect(TextTools.myParseDate('29.02.2019'), null); // 2019 is not leap year
    });
    ...
    test('date validation robustness - invalid years', () {
      ...
      expect(TextTools.myParseDate('15.06.2101'), null); // Year too far in future
    });
    
    test('date validation robustness - leap year handling', () {
      // Valid leap year dates
      expect(TextTools.myParseDate('29.02.2020'), DateTime(2020, 02, 29)); // 2020 is leap year
      expect(TextTools.myParseDate('29.02.2000'), DateTime(2000, 02, 29)); // 2000 is leap year (divisible by 400)
      
      // Invalid leap year dates
      expect(TextTools.myParseDate('29.02.1900'), null); // 1900 is not leap year (divisible by 100, not 400)
      expect(TextTools.myParseDate('29.02.2021'), null); // 2021 is not leap year
    });
    
    test('date validation robustness - valid edge cases', () {
      // Valid dates that should still work
      expect(TextTools.myParseDate('31.01.2020'), DateTime(2020, 01, 31)); // January 31st
      expect(TextTools.myParseDate('31.03.2020'), DateTime(2020, 03, 31)); // March 31st
      expect(TextTools.myParseDate('30.04.2020'), DateTime(2020, 04, 30)); // April 30th (valid)
      expect(TextTools.myParseDate('28.02.2019'), DateTime(2019, 02, 28)); // Feb 28 non-leap year
      
      // Year range boundaries
      expect(TextTools.myParseDate('01.01.1900'), DateTime(1900, 01, 01)); // Min year
      expect(TextTools.myParseDate('31.12.2100'), DateTime(2100, 12, 31)); // Max year
    });
  });
  }
