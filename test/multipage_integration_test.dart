...
void main() {
  ...
  group('Multi-page Append Integration Tests', () {
    ...
    test('analyzeAppendScenario works with test data', () async {
      ...
      expect(controller.participants.isNotEmpty, true, reason: 'Should have test participants');
  ...
  group('Database Flag Persistence Tests', () {
    ...
    test('participant printed flag persists and can be re-fetched', () async {
      ...
      expect(participants.isNotEmpty, true, reason: 'Should have test participants');
      ...
      // Set printed flag
      final success = await service.setParticipantPrintedFlag(participantId, true);
      expect(success, true);
      
      // Re-fetch and verify persistence
      final updatedParticipants = await db.getParticipantsByCurrentEvent();
      final updatedParticipant = updatedParticipants.firstWhere((p) => p.id == participantId);
      
      expect(updatedParticipant.wasPrinted, true, 
        reason: 'Participant wasPrinted flag should persist after database update');
    });
    ...
    test('record printed flag persists and can be re-fetched', () async {
      ...
      final updatedRecord = updatedRecords.firstWhere((r) => r.idZaznamu == recordId);
