...
void main() {
  group('NewRecordPage Comprehensive Tests', () {
    ...
    Future<MemoryOsoba> createTestParticipant(String firstName, String lastName) async {
      final participant = await database.into(database.participants).insertReturning(ParticipantsCompanion(
    ...
    group('Form Validation Tests', () {
      testWidgets('should prevent saving when no participant is selected', (WidgetTester tester) async {
        await tester.pumpWidget(
          MaterialApp(home: const NewRecordPage()),
        );
        await tester.pumpAndSettle();
  ...
  final saveButton = find.byKey(const Key('save_button'));
    ...
    group('Unsaved Changes Detection', () {
      testWidgets('should show unsaved changes indicator when typing', (WidgetTester tester) async {
        if (testParticipants.isNotEmpty) {
          final participant = testParticipants.first;
  ...
  expect(find.byKey(const Key('datetime_change_button')), findsOneWidget);
      ...
      testWidgets('should allow datetime changes when participant is selected', (WidgetTester tester) async {
        if (testParticipants.isNotEmpty) {
          final participant = testParticipants.first;
          ...
          await tester.pump(); // Just pump once to start the date picker opening
  ...
  expect(find.byKey(const Key('participant_autocomplete')), findsOneWidget);
    ...
    group('Complex Scenario Combinations', () {
      testWidgets('should handle form filling and validation properly', (WidgetTester tester) async {
        if (testParticipants.isNotEmpty) {
          final participant = testParticipants.first;
